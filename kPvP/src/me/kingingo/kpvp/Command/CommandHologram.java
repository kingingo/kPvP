...
public class CommandHologram implements CommandExecutor{
	...
	public CommandHologram(kPvPManager manager){
		this.manager=manager;
		this.config=new kConfig(new File("plugins"+File.separator+manager.getPvP().getPlugin(manager.getPvP().getClass()).getName()+File.separator+"holograms.yml"));
		...
		if(p.isOp()){
			if(args.length==0){
				...
				if(args[0].equalsIgnoreCase("today")){
					...
					kPvPListener.ranking_day.remove();
					kPvPListener.ranking_day=manager.getPvP().getHologram().sendText(CommandHologram.getToday(), manager.getStatsManager().getRankingMessage(CommandStats.getRanking_day(),"Tag"));
					...
					kPvPListener.ranking_month.remove();
					kPvPListener.ranking_month=manager.getPvP().getHologram().sendText(CommandHologram.getMonth(), manager.getStatsManager().getRankingMessage(CommandStats.getRanking_month(),"Monat"));
					...
					kPvPListener.ranking_week.remove();
					kPvPListener.ranking_week=manager.getPvP().getHologram().sendText(CommandHologram.getWeek(), manager.getStatsManager().getRankingMessage(CommandStats.getRanking_week(),"Woche"));
					...
					kPvPListener.ranking_total.remove();
					kPvPListener.ranking_total=manager.getPvP().getHologram().sendText(CommandHologram.getTotal(), manager.getStatsManager().getRankingMessage(CommandStats.getRanking()));
